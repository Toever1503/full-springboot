...
public interface IReviewRepository extends JpaRepository<ReviewEntity, Long>, JpaSpecificationExecutor<ReviewEntity> {
    ...
    @Query(value = "update tbl_product as p\n" +
            "set rating = (select sum(rv.rating)/count(rv.review_id) from tbl_review as rv where rv.product_id=?1 and parent_id is null)\n" +
    ...
    @Query("select r from ReviewEntity r where r.orderDetail.order.id = ?1 and r.createdBy.id = ?2")
    List<ReviewEntity> findAllMyReview(Long productId, Long userId);
}
